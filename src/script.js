...
function updateLiveText() {
  ...
  document.getElementById('live-text').innerText = `◦ Live ${date} ${time}`;
}
...
// Realistic planet sizes in comparison to Earth
const planetSizesReal = {
  'Mercury': 1,   // Relative to Earth (scaled down for better visibility)
  'Venus': 0.6,
  'Earth': 1,
  'Mars': 2,
  'Jupiter': 1.12,
...
// Scaled-down sizes for better visibility
const planetSizesScaled = {
  'Mercury': 0.24,
  'Venus': 0.61,
  'Earth': 0.64,
  'Mars': 0.34,
...
// Function to update planet sizes based on real size toggle
function updatePlanetSizes(realSize) {
  mercury.planet.scale.setScalar(realSize ? planetSizesReal['Mercury'] : planetSizesScaled['Mercury']);
  venus.planet.scale.setScalar(realSize ? planetSizesReal['Venus'] : planetSizesScaled['Venus']);
  earth.planet.scale.setScalar(realSize ? planetSizesReal['Earth'] : planetSizesScaled['Earth']);
  mars.planet.scale.setScalar(realSize ? planetSizesReal['Mars'] : planetSizesScaled['Mars']);
  jupiter.planet.scale.setScalar(realSize ? planetSizesReal['Jupiter'] : planetSizesScaled['Jupiter']);
  saturn.planet.scale.setScalar(realSize ? planetSizesReal['Saturn'] : planetSizesScaled['Saturn']);
  uranus.planet.scale.setScalar(realSize ? planetSizesReal['Uranus'] : planetSizesScaled['Uranus']);
  neptune.planet.scale.setScalar(realSize ? planetSizesReal['Neptune'] : planetSizesScaled['Neptune']);
  pluto.planet.scale.setScalar(realSize ? planetSizesReal['Pluto'] : planetSizesScaled['Pluto']);
}

// Event listener for real size toggle
document.getElementById('real-size-toggle').addEventListener('change', function() {
  const realSize = this.checked;
  updatePlanetSizes(realSize);  // Update planet sizes based on checkbox
});

// Call the function initially to set scaled size on load
updatePlanetSizes(false);  // Default to scaled size
